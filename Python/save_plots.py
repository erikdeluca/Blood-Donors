...
import hmm_glm_plots as hmm_pl  # noqa: E402
import hmm_glm_viterbi as viterbi  # noqa: E402
import hmm_glm_prediction as pred  # noqa: E402
...
T = len(year_cols)
obs = (
    df.select(year_cols).fill_null(0).to_numpy().astype(int)  # (N,T)
)
...
# ----------------------
W_pi, W_A, pi_base, A_base, beta_em = hmm_glm.load_hmm_params(
...
cov_names_pi = ["intercept", "birth_year_norm", "gender_code"]


age_fac_cols = hmm_pl.expand_factor_names(
    "age_years", age_years_levels, ref_level=ref_age_level
)
cov_names_A = ["intercept"] + age_fac_cols + ["covid_years"]
# cov_names_A = ["intercept"] + ["covid_years"]

em_age_fac_cols = hmm_pl.expand_factor_names(
    "age_years", age_years_levels, ref_level=ref_age_level
)
...
hmm_pl.plot_initial_probs(
    initial_probs=pi_base, state_names=state_names, colors=colors, ax=ax
)
...
paths_np = (
    paths.detach().cpu().numpy() if hasattr(paths, "detach") else np.asarray(paths)
)
N, T = paths_np.shape
...
for k in range(K):
    for t in range(T):
        rows.append(
            {"t": (t + 2009), "state": f"state {k}", "share": float(props[k, t])}
        )
...
fig_pi.savefig(
    here("thesis/img/hmm/pi_coeff.png"), dpi=300, bbox_inches="tight", transparent=True
)
...
fig_pi_birth.savefig(
    here("thesis/img/hmm/pi_age.png"), dpi=300, bbox_inches="tight", transparent=True
)
...
for i in donors_predict:
    ...
    p = hmm_pl.plot_donor_gg(
        ...
        state_to_label={
            0: "Non donatore",
            2: "Donatore saltuario",
            1: "Donatore frequente",
        },
        colors={
            "Non donatore": colors[0],
            "Donatore saltuario": colors[1],
            "Donatore frequente": colors[2],
        },
        y_max=4,
    )
    p.save(
...
pl_acc.savefig(
    here("thesis/img/hmm/accuracy.png"), dpi=600, bbox_inches="tight", transparent=True
)
...
fig.savefig(
    here("thesis/img/hmm/glm_error.png"), dpi=600, bbox_inches="tight", transparent=True
)
fig, ax = plt.subplots(figsize=(10, 0.5))
ax = hmm_pl.plot_error_distribution(
    df_pred["error_glm_rounded"], "GLM", ax, theme_minimal=True
)
fig.savefig(
    here("slides/images/glm_error.png"), dpi=300, bbox_inches="tight", transparent=True
)

fig, ax = plt.subplots(figsize=(10, 2))
ax = hmm_pl.plot_error_distribution(
    df_pred["error_hmm_ms_rounded"], "HMM-GLM multi-state", ax
)
fig.savefig(
...
fig, ax = plt.subplots(figsize=(10, 2))
ax = hmm_pl.plot_error_distribution(
    df_pred["error_hmm_os_rounded"], "HMM-GLM one-state", ax
)
fig.savefig(
...
hmm_pl.plot_error_distribution(
    df_pred["error_glm_rounded"], "GLM", axs[0], theme_minimal=True
)
hmm_pl.plot_error_distribution(
    df_pred["error_hmm_ms_rounded"], "HMM-GLM multi-state", axs[1], theme_minimal=True
)
hmm_pl.plot_error_distribution(
    df_pred["error_hmm_os_rounded"], "HMM-GLM one-state", axs[2], theme_minimal=True
)
plt.tight_layout()
fig.savefig(
...
hmm_pl.plot_error_distribution(
    df_pred["error_glm_rounded"], "GLM", axs[0], theme_minimal=True, with_title=False
)
hmm_pl.plot_error_distribution(
    df_pred["error_hmm_ms_rounded"],
    "HMM-GLM mistura",
    axs[1],
    theme_minimal=True,
    with_title=False,
