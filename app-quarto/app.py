...
# Osservazioni (N,T)
obs = df.select(year_cols).fill_null(0).to_numpy().astype(int)
N = obs.shape[0]
...
age_bins = np.array(
    [18, 25, 35, 45, 55, 60, 65, 75]
)  # 7 classi → 6 dummies (saltiamo baseline)
...
cov_tran = np.concatenate(
    [intercept_tile, ages_onehot, covid_years[:, :, None]], axis=2
)  # (N,T,8)
...
PATHS_ALL = viterbi.viterbi_paths_glm(
    obs_torch, cov_init_torch, cov_tran_torch, cov_emiss_torch, MODEL_PATH
...
def server(input: Inputs, output: Outputs, session: Session) -> None:
    ...
    ui.input_radio_buttons(
        "gender", "Genere", choices=["M", "F"], selected="M", inline=True
    )
    ...
    def parse_counts(text):
        return (
            [int(float(x)) for x in text.replace(",", " ").split() if x.strip()]
            if text.strip()
            else []
        )
    ...
    def plot_pmf_next():
        ...
        p = (
            ...
            + pn.scale_y_continuous(
                labels=lambda l_lambda: [f"{v*100:.0f}%" for v in l_lambda],
                limits=(0, max(probs) * 1.25),
            )
            + pn.labs(
    ...
    def plot():
        ...
        if not res:
            return hmm_pl.plot_donor_simple(
                years=[], donations=[], title="Inserisci i dati al lato per simulare."
            )
        return hmm_pl.plot_donor_simple(
            years=res["years"],
    ...
    ui.input_select(
        "donor_uid", "Donatore", choices=choices_map, selected=DEFAULT_UID, width="100%"
    )
    ...
    def vb_last_state():
        ...
        return ui.value_box(title="Stato latente (ultimo)", value=str(last_state))
    ...
    def vb_prob_next():
        ...
        return ui.value_box(title="P(donare next)", value=val_txt)
    ...
    def vb_expected_next():
        ...
        return ui.value_box(title="E[y next]", value=val_txt)
...
_static_assets = [
    "dashboard_files",
    "dashboard_files\\libs\\quarto-html\\tippy.css",
    "dashboard_files\\libs\\quarto-html\\quarto-syntax-highlighting-37eea08aefeeee20ff55810ff984fec1.css",
    "dashboard_files\\libs\\quarto-html\\quarto-syntax-highlighting-dark-2fef5ea3f8957b3e4ecc936fc74692ca.css",
    "dashboard_files\\libs\\bootstrap\\bootstrap-icons.css",
    "dashboard_files\\libs\\bootstrap\\bootstrap-366232e161007004ec4c38bee3e806d4.min.css",
    "dashboard_files\\libs\\bootstrap\\bootstrap-dark-366232e161007004ec4c38bee3e806d4.min.css",
    "dashboard_files\\libs\\clipboard\\clipboard.min.js",
    "dashboard_files\\libs\\quarto-html\\quarto.js",
    "dashboard_files\\libs\\quarto-html\\tabsets\\tabsets.js",
    "dashboard_files\\libs\\quarto-html\\popper.min.js",
    "dashboard_files\\libs\\quarto-html\\tippy.umd.min.js",
    "dashboard_files\\libs\\quarto-html\\anchor.min.js",
    "dashboard_files\\libs\\bootstrap\\bootstrap.min.js",
    "dashboard_files\\libs\\quarto-dashboard\\quarto-dashboard.js",
    "dashboard_files\\libs\\quarto-dashboard\\stickythead.js",
