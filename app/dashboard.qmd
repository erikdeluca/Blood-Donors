...
@render.ui
def vb_prob_next():
    val_txt = f"{res['prob_donate_next']:.3f}" if res['prob_donate_next'] is not None else "NA"
    return ui.value_box(
        title="P(donare next)",
        value=res['prob_donate_next']
    )
```
...
def vb_expected_next():
    val_txt = f"{res['expected_next']:.3f}" if mu_next is not None else "NA"
    return ui.value_box(
        title="E[y next]",
        value=res['expected_next']
    )
``` 
-->
...
def pred_info():
    try:
        ...
        return ""
    return ui.HTML(
        f"<b>E[y next]:</b> {res['expected_next']:.2f}<br>"
        f"<b>P(donare next]:</b> {res['prob_donate_next']:.1%}<br>"
        f"<b>Distribuzione next:</b> {res['pmf_next']}"
    )
``` 
-->


# Predizioni dal dataset
