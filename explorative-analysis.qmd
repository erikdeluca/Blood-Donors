...
## How many times has a donor donated?
...
data |>
  ...
  geom_hex(aes(fill = class_year), alpha = .2) +
  geom_smooth(aes(color = class_year), linetype = "dashed") +
  guides(color = "none")
```
...
data |> 
  ...
  scale_x_sqrt(breaks = c(1, 2, 5, 10, 25, 50, 100, 200))
```


Maybe I should slice by max donations by person
