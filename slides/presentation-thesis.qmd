...
if (python_path == "") {
  try(reticulate::use_condaenv("blood-donors", required = FALSE), silent = TRUE)
  python_path <- Sys.which("python")
}else{
...
readRDS(here::here("R/fit_tpoisson.rds")) |>
  ...
  dotwhisker::dwplot(
    dot_args = list(size = 3, color = "#54403bff"),
    show_intercept = TRUE,
    whisker_args = list(size = 1, color = "#df9457ff"),
    vline = ggplot2::geom_vline(xintercept = 0, colour = "grey50", linetype = 2)
  ) +
    labs(
      title = "Poisson troncato in zero",
      subtitle = "Coefficienti del modello con intervalli di confidenza"
    )
```
:::
...
ggplot(donations, aes(x = donations, y = probability)) +
  ...
  geom_text(aes(label = scales::percent(probability)), vjust = -0.3, size = 6) +
...
<!-- Questa parte, però, richiederebbe anche una revisione dei successivi algoritmi implementati e usati per le diagnostiche del modello -->
