...
y_{n,t} \mid z_{n,t}=k \sim \mathrm{Poisson}\Big(
  \exp\Big(\beta_{em,k} \cdot x^{em}_{n,t,m}
  \Big)
\Big)
$$
...
![](images/state_occupances.png)



## Coefficients on $\pi$

::: {layout-ncol=2 layout="[10,-1,15]"}
...
![](images/hmm_glm_coefficients_glm.png){width=80%}
...
\left\{
\begin{aligned}
& \alpha(z_1) = P(x_1 \mid z_1)\, P(z_1) \\
& \alpha(z_n) = P(x_n \mid z_n)\, \sum_{z_{n-1}} P(z_{n} \mid z_{n-1}) \,\alpha(z_{n-1})
\end{aligned}
\right.
$$
...
$$
P(z_{T+1}) = \alpha(z_n) \times A_{next}((x^{next})_A)
$$
...
**Next-state probabilities:** [0.371, 0.060, 0.569]  
...
|-------------|-------|-------|-------|-------|-------|------|
| Probability | 0.577 | 0.225 | 0.119 | 0.049 | 0.018 | 0.012 |
...
& \log \mu = \beta_0 + \beta_1 \,\text{birth_year_norm} + \beta_2 \,\text{gender}
\end{aligned}
\right.
$$
:::
