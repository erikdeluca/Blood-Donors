...
I modelli lineari generalizzati (GLM) sono un'estensione dei modelli di regressione lineare. Per comprendere appieno il significato di *Generalized Linear Model* si introduranno prima i modelli di regressione lineare e alcuni metodi di calcolo dei coefficienti del modello, gi elementi che lo caratterizzano. 
Successvivamente si passerà alla loro estensione e all'utilizzo della famiglia di distribuzioni esponenziale, citando alcune delle distribuzioni che successivamente verranno usate nei modelli.

### Regressione Lineare

Un modello è una rappresentazione matematica di un processo che genera dati, ovvero una semplificazione della realtà basata su assunzioni probabilistiche. 
Il modello si propone di spiegare una variabile, $Y$, attraverso delle variabili, $X$, che siano correlate con essa.
$Y$ verrà chiamata variabile dipendente, in quanto il suo valore dipenderà dal valore delle variabili $X$, che invece, verranno chiamate variabili indipendenti.

::: {.callout-note}
A causa della motlituidine di applicazioni in diversi ambiti della regressione lineare, non c'è una definizione univoca dei suoi elementi. Nel testo sarà comune chiamare la variabile $Y$ come variabile dipendente, *outcome*, *output* o *target*. Lo stesso discorso vale con la variabile, o le variabili, $X$, che possono essere chiamate variabili indipendenti, covariate, *features*, predittori o *inputs*.  
:::

<!-- Da rivedere la parte sotto -->
Una correlazione tra $X$ e $Y$ implica che $COV(X,Y) \neq 0$, dove la covarianza è definita come:
...
<!-- definire il modello di regressione lineare e le sue componenti, aggiungere le ipotesi e spostare la parte qua sotto in metodi di stima -->
Quando si desidera esaminare la relazione tra $X$ e $Y$, spesso si utilizza la regressione lineare, che si propone di spiegare la relazione tra $Y$ e $X$. 
La funzione di regressione di $Y$ su $X$, denotata come $E[Y|X]$, rappresenta il valore atteso di $Y$ dato $X$. Nella regressione lineare semplice, questa funzione è espressa come:
...
::: {.callout-note}
Se si considerasse l'intercetta nella matrice di covariate $X$, allora il termine $\alpha$ scomparirebbe e si potrebe scrivere l'equazione come 
$$
Y = X \beta + \varepsilon, \quad \varepsilon \sim N(0, \sigma^2)
$$

Per la stima di $\beta$ bisognerà minimizzare l'errore quadratico medio (MSE - *Mean Square Error*) per il criterio dei minimi quadrati (LSE - *Least Squared Error*):
$$
\text{MSE} = (Y - X\beta)'(Y-X\beta) 
$$
Bisognerà trovare quindi l'argomento $\beta$ che minimizzi l'MSE ed è risolto da:
$$
...
<!-- descrivere il metodo di stima dei minimi quadrati, spiegare come questo equivalga alla massima verosimglianza e dare una definizione di funzione di verosimiglianza e descrizione del metodo. Infine accennare ad altri metodi, come il metodo dei momenti -->

### Estensione della Regressione Lineare

<!-- spiegare la funzione legame (link), le ipotesi del GLM e le sue differenze con il LM -->
...
<!-- descrivere le differenze tra modello predittivo ed esplicativo -->
<!-- descrivere come è stato strutturato il dataset -->
