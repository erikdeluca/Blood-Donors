...
Per il punto di previsione in $T{+}1$ non usiamo la mistura sugli stati come fatto nel capitolo precedente (@sec-forward), ma selezioniamo lo stato più probabile in $T{+}1$, $j^\star = \arg\max_j\, p_{T+1}(j),$ e applichiamo il GLM di quello stato:
$$
...
![Accuracy dei modelli](img/hmm/accuracy.png){#fig-accuracy}

#### Analisi degli errori

Le mappe di calore in @fig-errori mostrano la distribuzione percentuale degli errori arrotondati $(\hat y - y)$. Per l’HMM-GLM (@fig-err_hmm) emerge una forte massa in 0 (predizione esatta) e, a seguire, in -1 e -2 (sottostima); per il GLM (@fig-err_glm) la maggior parte degli errori è tra -1 e 1 con dispersione più simmetrica.
...
::: {#fig-errori layout="[[1,1]]"}

![Errori del GLM](img/hmm/glm_error.png){#fig-err_glm}

![Errori dell’HMM-GLM](img/hmm/hmm_error.png){#fig-err_hmm}

Grafico a mappa di calore sulla frequenza degli errori per i due modelli testati
:::